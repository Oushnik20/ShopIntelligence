...
VIDEO_PATH = r"Store_video/ENTRY_CAM_2.mp4"
STORE_ID = "STORE_BLR_002"
CAMERA_ID = "ENTRY_CAM_2"
...
while True:
    ...
    if tracks.tracker_id is None:
        cv2.imshow("STORE2 ENTRY CAM 2", frame)
        if cv2.waitKey(1) == 27:
            break
        continue
    ...
    for box, track_id in zip(tracks.xyxy, tracks.tracker_id):
        ...
        if center_x > ENTRY_LINE_X and track_id not in crossed_in:
            ...
            save_entry_event(track_id, store_id=STORE_ID, camera_id=CAMERA_ID)
            print(f"STORE2 ENTRY SAVED -> VIS_{track_id}")

        if center_x <= ENTRY_LINE_X and track_id in crossed_in and track_id not in crossed_out:
            crossed_out.add(track_id)
            save_exit_event(track_id, store_id=STORE_ID, camera_id=CAMERA_ID)
            print(f"STORE2 EXIT SAVED -> VIS_{track_id}")

        cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
        cv2.putText(frame, f"ID {track_id}", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)

    cv2.line(frame, (ENTRY_LINE_X, 0), (ENTRY_LINE_X, frame.shape[0]), (0, 0, 255), 3)
    cv2.putText(frame, "ENTRY LINE", (ENTRY_LINE_X - 120, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
    cv2.imshow("STORE2 ENTRY CAM 2", frame)
